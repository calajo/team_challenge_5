...
def disparo_aleatorio_maquina(tablero_jugador):
    ...
    # === POSIBLE AÑADIDO: priorizar casillas cerquita de impactos previos === (lo comento todo por si hay lios)
    # Lo que va a hacer es "ver" o escanear el tablero de disparos del jugador,
    # y si ve un impacto, buscará posiciones ortogonales (arriba, abajo, izquierda, derecha)
    # que no hayan sido disparadas aún (AGUA), y las añadirá a una lista de candidatos.
    # Si encuentra candidatos, elegirá uno al azar de esa lista para disparar.
    candidatos = [] # lista de posiciones candidatas
    for f in range(DIMENSION_TABLERO): #Bucle por filas
        for c in range(DIMENSION_TABLERO): #Bucle por columnas
            if tablero_jugador.tablero_disparos[f][c] == IMPACTO:
                # vecinos ortogonales, vaya los que están cerca
                for df, dc in [(-1,0), (1,0), (0,-1), (0,1)]: #df y dc por desplazamiento fila/columna. Definimos los desplazamientos ortogonales
                    nf, nc = f + df, c + dc #nueva fila y nueva columna, calculamos la coordenada vecina
                    if 0 <= nf < DIMENSION_TABLERO and 0 <= nc < DIMENSION_TABLERO: #Chequeo de límites: aseguramos que (nf, nc) esta dentro del tablero
                        if tablero_jugador.tablero_disparos[nf][nc] == AGUA: #Si esa posición no ha sido disparada todavia
                            candidatos.append((nf, nc)) #la añadimos a la lista de candidatos, para priorizarla
    if candidatos: #Si hay candidatos disponibles
        return random.choice(candidatos) #Elegimos uno al azar y lo devolvemos
    # === FIN AÑADIDO ========================================================
